 ...
 * 3) Idol클래스는 아이돌 이름과 출생년도 정보를 들고있다.
 *    (name 프로퍼티, year 프로퍼티)
 * 4) MaleIdol 클래스는 Idol 클래스와 동일하게 
 *    name, year 프로퍼티가 존재한다. 
 *    추가로 sing() 메서드를 실행하면 ${이름}이 노래를 합니다.
 *    라는 스트링을 반환해준다.
 * 5) FemaleIdol 클래스는 Idol클래스와 동일하게
 *    name, year 프로퍼티가 존재한다.
 *    추가로 dance() 메서드를 실행하면 ${이름}이 춤을 춥니다.
 *    라는 스트링을 반환해준다.
 * 
 * 
...
class MaleIdol extends Idol {
    sing() {
        return `${this.name}이 노래를 합니다.`; // js에서는 부모클래스의 프로퍼티 가져올때 super가 아닌 this로 접근하여 가져옴
    }
...
class FemaleIdol extends Idol {
    dance() {
        return `${this.name}이 춤을 춥니다.`;// js에서는 부모클래스의 프로퍼티 가져올때 super가 아닌 this로 접근하여 가져옴
    }
